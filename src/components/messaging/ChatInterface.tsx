...
export function ChatInterface({
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Input */}
      <div className="bg-white border-t border-green-200 p-4 shrink-0">
        <div className="flex items-center gap-2 mb-3">
          <label className="text-sm font-medium text-gray-700">Auto-delete:</label>
          <select
            value={expirationHours || ''}
            onChange={(e) =>
              setExpirationHours(e.target.value ? parseInt(e.target.value, 10) : undefined)
            }
            className="text-sm border border-gray-300 rounded-lg px-3 py-1.5 bg-white hover:border-green-400 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all"
          >
        ...
        <div className="flex items-center gap-3">
          <input
            ...
            className="flex-1 h-12 border-2 border-gray-200 rounded-xl px-4 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 hover:border-gray-300 transition-all shadow-sm"
          />
          <button
            onClick={handleSendMessage}
            disabled={!messageInput.trim() || sending}
            className="h-12 bg-green-600 text-white px-6 rounded-xl hover:bg-green-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center gap-2 shadow-lg hover:shadow-xl font-medium min-w-[120px] transform hover:scale-105 active:scale-95"
          >
            {sending ? (
              <>
